...
const Comparison = () => {
    ...
    const [colorSets] = useState(["#5EE3D0","#8388EE" , "#45B6D4" , "#FFC83A"])
    ...
    let profileUnit = <div className="unit-text">
...
let profileUnit_  = <div className="unit-text">
<div className="unit-row">
    <h4>Size</h4>
</div>  
<div className="unit-row">
...
const profileUnit2_ = <div className="unit-text">
<div className="unit-row">
    <h4>Size (cm)</h4>
</div>  
<div className="unit-row">
...
<div className="unit-row">
    ...
    useEffect(() => {
        ...
        Product.getProduct(search,priceRange,RockerType,widthType,setbackRange,lengthRange,setbackActive,brandsActive,switchoption,Radius).then((products,err)=>{
            if(!err){
...
// eslint-disable-next-line
    ...
    }, [search,priceRange,RockerType,widthType,setbackRange,lengthRange,setbackActive,brandsActive,switchoption,Radius])
            ...
            for (let i = 0; i < graphics.length; i++)
                {
                    ...
                    let sidecut_name;
                        ...
                        let elements = [ { amount : graphics[i].size , precentage : Math.floor(graphics[i].size / sizebase * 100) , unit : 'cm' , name : "Size"}  , 
                        { amount : graphics[i]["taper"] < 1 ? "0" : graphics[i]["taper"] , precentage : graphics[i]["taper"] < 1 ? 0 : Math.floor(graphics[i]["taper"] / taperbase * 100) , unit : 'mm' ,  name : "Taper" }, 
                        { amount : graphics[i].TipWidth , precentage : Math.floor(graphics[i].TipWidth / TipWidthbase * 100) , unit : 'mm' ,  name : "Tipwidth"},
                        { amount : graphics[i].WaistWidth , precentage : Math.floor(graphics[i].WaistWidth / WaistWidthbase * 100) , unit : 'mm' ,  name : "WaistWidth"}
                        ];

                        let elements2 =  
                            [
                               { amount : graphics[i]["size"] , precentage : Math.floor(graphics[i]["size"] / sizebase * 100) , name : "Size" }  , 
                               { amount : graphics[i]["taper"] == 0 ? "0" : graphics[i]["taper"] , precentage : graphics[i]["taper"] == 0 ? 0 : Math.floor(graphics[i]["taper"] / taperbase * 100) , unit : 'mm' ,  name : "Taper"}, 
        ...
        <section id="comparison-app" data-callpased={sidebarshow ? true : false}>
            ...
            >
                <div className='view-type'>
                    <a href="#" 
                    className={viewType == "Graphics" && !lineview ? 'view-active view-link' : "view-link"}
...
data-callpased={collapsible ? "true" : "false"} data-alignbottom={alignBottom ? "true" : "false"} ref={canvas}>
               ...
               {  !zoom ?  
                ...
                <div className="canvas-text" data-active={lineview ? "true" : "false"} data-callpaseds={collapsible ? "true" : "false"} data-graph={graphactive ? "true" : "false"} ref={canvas}>
                        {
                        Array.isArray(activeGraphics) && activeGraphics.length ?   switchoption == "Snowboard" ? profileUnit_ : profileUnit : null
                        } 
                        {
